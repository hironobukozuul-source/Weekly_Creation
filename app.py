...
def load_sg_cap_mappings(uploaded_file):
    """
    Reads the SG-CAP sheet (case-insensitive) to map: SKU Name (Col D) -> Filling Weight in Grams (Col G)
    """
    try:
        ...
        if not target_sheet:
            st.sidebar.error("Could not find 'SG-CAP' sheet in this file.")
            return {}
...
if uploaded_file:
    ...
    sg_cap_map = load_sku_conversion_table = load_sg_cap_mappings(uploaded_file)
    ...
    if available_weeks:
        ...
        if st.button("🚀 レポート生成開始"):
            with st.spinner('データを最適化処理中...'):
                ...
                # Pre-filter week parameters to speed up Excel matrix calculations 
                ...
                # Isolate only tasks falling inside the active timeline boundary
                ...
                # Vectorized processing instead of scanning empty elements
                ...
                for r_idx, (line, product) in enumerate(unique_items):
                    ...
                    # Filter matching records once per row instead of per cell
                    ...
                    for c_idx, h_dt in enumerate(hour_list):
                        ...
                        if ton_sum > 0:
                            ...
                            # Write pieces
                            if filling_grams:
                                total_pcs = (ton_sum * 1000000) / filling_grams
                                cell_p = ws_pcs.cell(row_num, 3 + c_idx, int(round(total_pcs)))
                            else:
                ...
                # Apply layout line breaks to grids
                for ws in [ws_vol, ws_pcs]:
