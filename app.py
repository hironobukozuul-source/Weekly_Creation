...
def generate_plot(df_tasks, start_date):
    ...
    for camp in merged:
        ...
        for s_dt, f_dt in camp['Segments']:
            ...
            if e > s:
                if is_m: 
                    ...
                    ax.hlines(y, s, e, colors=color, linewidth=10, capstyle='butt', zorder=3)
        ...
        if not is_m:
            ...
            line_offset_state[line_name] = -50 if y_off > 0 else 50
            ax.annotate(f"{camp['Product']}\n{camp['TotalTon']:.1f}t", xy=(mid, y), xytext=(0, y_off), textcoords='offset points', ha='center', va=('bottom' if y_off > 0 else 'top'), bbox=dict(boxstyle='square,pad=0.3', fc='white', ec=color, lw=1.5, alpha=0.9), arrowprops=dict(arrowstyle='->', color=color), fontsize=12, fontweight='bold', fontproperties=jp_font)

    # 2. ライン間の3時間数値ガイド (上下端のグレー帯を削除)
    for y_idx in range(len(plot_order) - 1):
        strip_y = y_idx + 0.5
        ax.axhline(strip_y, color='#F5F5F5', linewidth=18, zorder=1)
    ...
    ax.text(0.5, 1.12, f"Production Plan - Week of {start_date} (+6hrs)", 
            transform=ax.transAxes, fontsize=48, fontweight='bold', ha='center', va='center', fontproperties=jp_font)
    
    # 5. 承認ボックス (PM & SV) - 絶対座標でチャート外へ配置
    box_w, box_h = 0.033, 0.05
    pos_y = 0.88  # 上部余白内の位置
    # 指定のオフセット（330px左）を座標に換算
    new_pm_x = 0.883 - 0.011 
    ...
    fig.text(new_sv_x + (box_w/2), pos_y + box_h + 0.005, 'SV', transform=fig.transFigure, ha='center', fontweight='bold', fontsize=16, zorder=21)
    # PM Box
    ...
    fig.text(new_pm_x + (box_w/2), pos_y + box_h + 0.005, 'PM', transform=fig.transFigure, ha='center', fontweight='bold', fontsize=16, zorder=21)
...
if uploaded_file:
    ...
    if available_weeks:
        ...
        with col1:
            selected_week = st.selectbox("対象週（月曜日）を選択", available_weeks)
        with col2:
            st.write(" ")
            generate_btn = st.button("🚀 レポート生成", use_container_width=True)

        if generate_btn:
            with st.spinner('グラフとExcelを作成中...'):
                df_tasks = process_tasks(df_raw)
                img_buf, fig = generate_plot(df_tasks, selected_week)
                
                # Excel ワークブックの作成（データシート + 画像シート）
                wb = openpyxl.Workbook()
                # Hourly Volume シートの作成 (省略せずにロジック維持)
                ws_vol = wb.active
                ...
                # ... (Excelのロジックは前のバージョンを継承) ...
                
                # Visual Schedule シートに画像を貼り付け
                ...
                st.success(f"✅ {selected_week} 週のレポートが完成しました")
                st.download_button("📥 Excelレポートをダウンロード", out_excel.getvalue(), f"Production_Report_{selected_week}.xlsx")
